...
### 6-12k　訊號不好時斷線，畫面上看不出來

**先講好消息：斷線期間的訊息不會遺失。** EventSource 自己會重連，
而 Firebase 在（重新）連上時會重送整個節點，`seen` 這個集合負責去重，
所以斷線那段時間對方講的話會在重連後一次補回來，也不會重複。

**壞消息是使用者不知道自己斷線了。** 兩個缺陷：

| 缺陷 | 後果 |
| --- | --- |
| 通話中的兩條串流**完全沒有錯誤處理**（`fbStream` 只傳了兩個參數） | 講到一半斷線，畫面上一點跡象都沒有，繼續對著沒連線的電話講 |
| 待機的「重新連線中」設了**就不會恢復**（只監聽 error，沒監聽 open） | 斷一次那行字就一直掛著騙人 |

**解法**：`fbStream` 補上 `onOpen`，斷了要講、回來了也要講。

通話狀態改成統一走 `Call.showStatus()`，因為**斷線訊息不能被
「通話中」「翻譯中」蓋掉**——那正是使用者最需要看到它的時候。
兩條串流任一條斷了就算斷線，**兩條都回來才算恢復**。

> 這一段的教訓：`fbStream` 的第三個參數是選填的，所以少傳不會報錯，
> 只會安靜地少掉錯誤處理。呼叫兩次、其中一次忘了傳，就這樣過了很久。

### 6-12l　字太小，而且手機的「較大文字」完全無效

**查證的起點**：先量了 11 組前景／背景的對比，**全部通過 WCAG AA**
（最低的 4.62:1）。所以顏色不用動——先確認再說，免得改了本來就對的東西。

**真正的問題是字級全部寫死成 px**（32 處，rem 零處）。後果是
**手機系統的「較大文字」設定對這個 App 一點作用都沒有**，
看不清楚的人只能瞇著眼睛看。

**解法**：

1. 32 處 `font-size` 全部改成 `rem`。
2. 根字級用**百分比**而不是 px：
   ```css
   html { font-size: 100%; }      /* 100% = 使用者自己設定的大小 */
   html.text-l  { font-size: 112.5%; }
   html.text-xl { font-size: 125%; }
   ```
   **寫成 `16px` 就等於再一次覆蓋掉系統設定**，那就白改了。
3. 設定頁加「字級」（標準／大／特大），**選了立刻生效**——
   跟介面語言一致；同一頁上兩種行為會讓人以為設定跑掉了（見 6-13）。

順手補了一個不一致：`.btn.text`（試唸、試聽、全部刪除）的點擊區只有約 35px，
但同一個檔案裡 `.icon-btn` 的註解就寫著「44 是手指好按的最小尺寸」。已補到 44。

> 特大字下的版面有實際截圖確認過：泡泡正確換行、沒有橫向捲軸、按鈕沒被擠掉。
> 測試也把「有沒有跑出橫向捲軸」當成爆版的判斷條件。

### 6-12m　來電鈴聲太小聲聽不到

**現象**：實際使用回報「太小聲聽不到，要大聲一點」——漏接一通就是漏掉一筆生意。

**原因**：音量寫死 `0.22`，而且**單純把這個數字調大會變成爆音**。
鈴聲是 440 與 480 兩個振盪器疊在同一個 gain 上，**實際峰值是設定值的兩倍**，
直接送出去超過 0.5 就削頂，聽起來刺耳又更不像電話鈴聲。

**解法**：輸出前串一顆限幅器（`DynamicsCompressor`，門檻 −8dB、比例 12），
把疊加後的峰值壓住，音量就推得上去而不會破。

| | 舊 | 新 |
| --- | --- | --- |
| 音量 | 寫死 0.22 | 五段 0.20 / 0.40 / 0.70 / 0.95 / 1.20 |
| 預設 | — | **第 5 段**（約原本的 5.5 倍） |
| 回鈴音 | 寫死 0.09 | 跟著設定走，固定為來電的 40% |

設定頁多一條「鈴聲音量」滑桿，**試聽用滑桿的當下數值**，不必先按儲存。
最小段比原本更小，晚上在家可以調小。

> 沒有 `createDynamicsCompressor` 的瀏覽器（極少數）會直接輸出——
> 聲音小一點總比完全沒鈴聲好。

**如果還是聽不到，先檢查手機這兩件事**（不是程式能控制的）：

- **媒體音量**。網頁的聲音走的是「媒體」音量，不是「鈴聲」音量，兩者是分開的。
- **iPhone 的靜音實體開關**。撥到靜音時網頁聲音會被壓掉。

### 6-12n　電腦上放捷徑，來電沒反應

**現象**：在 PC 上做了捷徑放在工作列，來電時完全沒反應。

**原因不是設定沒做對，是「捷徑」本身做不到這件事**。捷徑只是個啟動器——
點下去才開網頁。**視窗關著的時候，沒有任何東西在跑**，來電當然收不到。

手機上不會遇到，因為「加到主畫面」裝的是 PWA，背景服務會留著等推播。

**解法只要三步，不麻煩**：

1. 用 Chrome／Edge 開網頁 → 網址列右邊會有一個**安裝圖示**（⊕ 或螢幕加箭頭）
   → 按「安裝」。沒看到就走選單 → 「投放、儲存及分享」→「安裝頁面為應用程式」。
2. 從**新的**應用程式圖示開啟（舊捷徑砍掉，不然會分不清），按一次
   **「啟用鈴聲與通知」**，允許通知。
3. 要開機就自動等來電：Windows 設定 → 應用程式 → 啟動，把它打開。
   （或把它放進「啟動」資料夾：`Win+R` → `shell:startup`）

> **視窗可以最小化，但不能整個關掉。** 瀏覽器本身也要留在背景——
> Chrome 的「關閉後繼續執行背景應用程式」打開會比較穩。

**程式端補上的提示**（2026-09-01）：在電腦上、而且還沒安裝成應用程式時，
待機卡片會直接寫出這件事。原本只處理了 iPhone 的情況，電腦完全沒提示，
使用者只會看到「來電沒反應」而不知道為什麼。

### 6-14　每月密碼登入

**先講清楚它擋得住什麼、擋不住什麼。**

驗證在瀏覽器裡做，網頁原始碼誰都看得到。程式裡存的是 SHA-256 不是明碼，
所以「檢視原始碼」不會一眼看到十二組密碼——**但六位英數的密碼，
用程式跑幾秒就能反推**。

| | |
| --- | --- |
| ✅ 擋得住 | 拿到網址就亂用的人。不讓陌生人耗掉翻譯額度、塞爆資料庫 |
| ❌ 擋不住 | 有心破解的人。要真正的存取控制，必須改成伺服器端驗證 |

以「不想被隨便撿到網址的人白用」這個目的來說，它是夠的。

**行為**：

- 沒通過就停在登入頁，**連線、推播註冊都不會先做**——沒登入的人不該
  佔用任何資源，也不該把自己掛上待機。
- 通過之後記住，**換月才會再問一次**。這是每月換密碼的重點。
- 大小寫、前後空白都不計較（使用者是用手打的，不該為了這個被擋）。
- 登入頁會寫出「目前是幾月」。**手機日期跑掉時，這一行是唯一查得出來的線索**——
  沒有它，使用者只會覺得「密碼明明對的卻進不去」。

**要換密碼**：

```
python3 tools/make-passwords.py <十二組，一月到十二月>
python3 tools/make-passwords.py --random          # 隨機產生
```

它會把明碼印出來（**抄走保存，程式裡只留雜湊，之後查不回來**），
然後改寫 `web/app.js` 裡的 `HASHES`。改完照常跑版本戳記再部署。

> ⚠️ 密碼清單不要寫進會公開的地方。目前這份筆記在 repo 裡，
> repo 是私有的才安全。

**目前這批**（2026-09 設定，明碼只記在這裡）：
...
| 訊號不好時對方好像收不到、畫面沒反應 | 斷線時通話畫面沒有任何提示（已修正） | 見 6-12k |
| 字太小、手機調大字沒有用 | 字級寫死成 px（已修正，設定頁可調） | 見 6-12l |
| 來電鈴聲太小聲 | 音量寫死且受限於削頂（已修正，可調五段） | 見 6-12m |
| 電腦上放捷徑但來電沒反應 | 捷徑只是啟動器，視窗關著就沒東西在跑 | 見 6-12n |
| 客戶說密碼進不去 | 手機日期跑掉，或拿到上個月的密碼 | 看登入頁寫的「目前是幾月」，見 6-14 |
